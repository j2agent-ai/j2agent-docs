...
- `collection_name`：必填。该 `info.json` 前缀目录下的知识库文档写入的 Milvus collection。名称只能包含英文字母、数字和下划线（`_`），不能包含连字符（`-`）、空格等特殊字符。例如应使用 `j2agent_docs`，不能使用 `j2agent-docs`。
...
```text
io.milvus.v2.exception.MilvusClientException: Invalid collection name: j2agent-docs. collection name can only contain numbers, letters and underscores: invalid parameter
```

请将对应 `info.json` 中的 `collection_name` 改为合法名称（例如 `j2agent_docs`），然后重新执行知识库同步。
