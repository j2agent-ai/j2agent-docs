...
多个 `info.json` 可以写入同一个 `collection_name`，也可以写入不同 collection。

当多个一级知识库目录或多个并列 `info.json` 使用相同 `collection_name` 时，它们会写入同一个 Milvus 物理 collection，即查询侧会在同一个 collection 中融合检索这些目录的内容。系统不会因为一级目录不同而自动拆分 collection；是否融合由 `info.json.collection_name` 决定。

融合并不表示文件身份被合并：`source_file` 保存的是相对 `knowledge-repo` 根目录的路径，例如 `wiki/home.md`、`manual/home.md`。因此不同一级目录下同名文件不会覆盖；增量删除、修改和 `collection_name` 迁移也仍按各自 `source_file` 精确清理。collection 的物理回收以“当前是否还有 ACTIVE 文件映射到该 collection”为准，只要任一目录仍有 ACTIVE 文件，collection 就不会被 drop。

## 2.1 知识库列表与远程仓库配置

管理端“知识库列表”统一展示 `knowledge-repo` 根目录下的一级目录：

- 有远程配置的一级目录显示为远程知识库，第一版协议为 Git。
- 没有远程配置的一级目录显示为文件知识库，只读展示。
- 远程配置只负责文件更新，不保存 collection、分片策略等知识库业务信息；这些信息仍完全由该目录内命中的 `info.json` 控制。
- 远程 Git 知识库通过 JGit 拉取，不依赖操作系统安装 Git。
- 新增远程知识库时，`repoCode` 对应 `knowledge-repo/{repoCode}` 一级目录；未填写时由 Git URL 的仓库名推导。
- 分支名可不填；不填时使用远端默认分支。填写时按指定分支同步。
- 同步为异步提交：创建、手动更新、定时更新都会先提交后台任务，列表状态显示同步进度。
- 更新周期仍以分钟保存，后端使用 `@Scheduled(cron = "0 * * * * *")` 每分钟在系统时间第 0 秒扫描到期远程仓库，再按各仓库 `update_interval_minutes` 判断是否提交同步。
- Git 同步语义严格对齐远端：首次 clone；后续 fetch 后 checkout/reset 到远端分支，并 clean 本地多余文件。
- 删除远程知识库会同时删除 DB 远程配置和 `knowledge-repo/{repoCode}` 本地目录，然后提交一次增量同步以清理已入库文件。
- 若因磁盘、误删、部署迁移等原因导致 DB 中有远程配置但本地一级目录不存在，列表仍会显示该远程知识库，状态为“目录缺失”；允许点击“更新”重新 clone 拉取。
...
- 若文档没有 `#`、`##`、`###` 标题，但启用了 `filename_as_title=true`，则以文件名作为唯一标题，全文作为 `text_chunk` 生成一个逻辑块。
...
3. 若检测到 Embedding 与 Milvus 不一致 → exclusive 完全重建；否则 `initializeHashCache()` 加载 `knowledge_source_file_hash` 后 `executeIncrementalSync` 首轮增量
4. `watch-enabled` 时 `startWatch` → 事件回调 `requestIncrementalSync("watch")`（probe 失败时不启动 watch）
