...
| 触发源 | 入口 | 任务类型 |
|--------|------|----------|
| 应用启动 | `requestStartupInit()` | `INITIALIZING` → exclusive 完全重建 |
...
| 触发方式 | 说明 |
|----------|------|
| 应用启动 | `requestStartupInit()`：停止任务 → drop collection → 全量 re-embed |
...
以上三种路径统一走 exclusive 完全重建：**停止所有维护任务 → drop 知识库绑定 collection → 重建 Milvus 客户端（清 SDK schema 缓存）→ probe 当前启用 Embedding 维度 → 全量 re-embed**。
...
- 可强占进行中的 exclusive 完全重建（与启动 / Embedding 变更相同）
...
| 规则 | 行为 |
|------|------|
| 启动 / Embedding 变更 / 手动完全重建 | **互相强占**：cancel + await idle → 最新 generation 继续 drop + resetClient + probe + re-embed |
