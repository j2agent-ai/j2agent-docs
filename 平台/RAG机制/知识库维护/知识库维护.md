...
- 每个标题块生成一个 **逻辑 text_chunk**：
  ...
  - `text_chunk`：标题下正文，存 MySQL `LONGTEXT`；**正文为空时以标题全文作为 text_chunk**。
...
作用：持久化标题块完整正文（LONGTEXT），供列表展示与检索结果回填；Milvus 仅存向量与窗口切片文本。
...
入库向量字段 `text` 受 Milvus **8192 UTF-8 字节**限制（`MilvusKnowledgeWriteService` 过滤）；完整正文不受此限，存 MySQL `text_chunk`。
...
**Schema 变更（如 question/answer → type、MySQL 正文分离）后首次上线也必须完全重建。**
