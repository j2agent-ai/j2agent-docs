...
本文说明 RAG **查询侧**如何将**经 QueryTransformer 链预处理后的** query 文本转为向量、在 Milvus 中执行 **稠密 + 稀疏混合检索**，以及超长问题 **多段向量化与结果融合**。Query 预处理见 [Query预处理.md](Query预处理.md)；对话侧传入 Milvus 的 `queryText` **不是**用户原文，而是 Transformer 链输出。
...
| 重叠 | 与入库共用 `j2agent.knowledge.repo.content-segment-overlap-chars`（见 `application.yaml`） |
| 最大段数 | 默认 **4**；末段槽位用于 **全文尾部**（保留长文末尾意图） |
| 断点 | 段尾优先在 `\n` 处切开 |

单段上限配置前缀：`j2agent.retrieve`；重叠配置前缀：`j2agent.knowledge.repo`。
...
- **去重键**：优先 `textChunkId`（同一逻辑 text_chunk 的 title / content_segment 多向量合并）；其次 `hash`（segment_id）。
- **正文回填**：去重融合后按 `textChunkId` 从 PostgreSQL `knowledge_text_chunk` 加载完整 `textChunk` 供 RAG / 命中测试。
...
j2agent:
  knowledge:
    repo:
      # 入库 content_segment 与超长 query 切分共用的相邻段重叠字符数
