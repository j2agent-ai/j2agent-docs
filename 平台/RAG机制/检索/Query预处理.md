...
- **Multimodal VLM**：构建 ChatModel 与每次请求的 `ChatOptions` 均 **OFF**（Anthropic `thinking: disabled`、Ollama `disableThinking`、open-ai `reasoning_effort=low`、LM Studio `reasoning_effort=low`；vLLM 不传参）
- 不读取会话级 thinking 覆盖
