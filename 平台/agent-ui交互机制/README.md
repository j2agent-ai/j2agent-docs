...
- `IDLE`：未开始。
- `AGENT_ORCHESTRATING`：通用助手编排服务 开放召回 / 决策阶段；仅 `universal_assistant` 且存在可调用子智能体、且有候选时出现。
- `THINKING`：模型思考中。
...
- `IDLE -> THINKING`：接收用户消息（非 universal 或无子智能体时）。
- `IDLE -> AGENT_ORCHESTRATING -> THINKING`：`universal_assistant` 被动意图召回完成后进入 ReAct。
...
- `THINKING/STREAMING_TEXT/AGENT_ORCHESTRATING/CALLING_TOOL/LOAD_SKILL -> CANCELLED`：用户中断或连接关闭。
...
stateDiagram-v2
    [*] --> IDLE
    IDLE --> THINKING: 接收用户消息（通用路径）
    IDLE --> AGENT_ORCHESTRATING: universal 编排服务
    AGENT_ORCHESTRATING --> THINKING: 编排完成
    ...
    AGENT_ORCHESTRATING --> CANCELLED: 用户中断/连接关闭
...
- `eventType=MESSAGE`：承载文本流式数据（目前复用 `ChatResponseDto`）。
  - **增量文本**：`phase=DELTA`，`message.content` / `reasoningContent` 追加。
  - **侧信道 PATCH**：`phase=PATCH`，可携带 `message.actions`（NMS navigate 动作卡片，见 [UI 导航](../UI导航/README.md)）、`message.pendingQuestion`（平台提问卡片）或 `message.srcFile`（RAG 来源）；前端 `applyMessageDelta` 合并写入当前 assistant 气泡，不替换已有字段。
- `eventType=TOOL`：工具调用生命周期数据；`read_skill` 同样使用该类型，但状态机处于 `LOAD_SKILL`，载荷中可带 `skillName` 供 UI 展示。
- `eventType=SYSTEM/NOTICE`：状态提示与诊断信息。

### 3.2.1 平台级提问卡片（`ask_question`）

`ask_question` 是平台统一注入的澄清工具，用于 Agent 在关键信息不确定时向用户提问，并等待用户选择候选答案或填写自定义回答后继续。Agent 应先完成本轮必要的理解、判断、工具结果整理或可确定部分说明；确认仍缺少用户选择/补充信息且本轮即将结束时，再调用 `ask_question`。
...
- `pendingQuestion.type` 固定为 `ask_question`。
- `version` 当前为 `1`。
- `question` 为面向用户的简短问题。
- `options` 只包含真实业务候选答案；“自定义回答”由前端固定提供，不由模型生成。
- 用户回答后发起新用户消息，消息内容仅为答案文本本身，不拼接原问题。

**后端链路**：

1. `AiAgent` 默认启用 `ask_question`，通过 `enableAskQuestion()` 控制；特殊 Agent 可 override 返回 `false`。
2. `AskQuestionTool` 手写实现 `ToolCallback`，由基类直接注入，发布问题后正常返回“立即结束本轮对话”的指令，让模型自然收尾等待用户回答。
3. 工具校验 `question/optionsJson`，过滤空选项与“自定义/其他/以上都不是”等 UI 兜底项。
4. `TurnAskQuestionRegistry` 将 `pendingQuestion` 以 `MESSAGE/PATCH` 发送到当前 assistant 气泡，同时缓存本轮 payload 供落库。
5. 工具发布成功后返回“立即结束本轮对话、等待用户回答”的工具结果；模型按提示自然停止，正常进入回合收尾，不通过 `CANCELLED` 表示提问等待。
6. assistant 落库 metadata 保留 `pendingQuestion`，历史回放时仍能看到该轮触发过提问。

**前端交互**：

- `dispatcher.applyMessageDelta` 合并 `message.pendingQuestion` 到当前 assistant 消息；空 assistant 气泡只要带 `pendingQuestion` 就不会被清理。
- `ChatView.vue` 在气泡内渲染 `AskQuestionCard`，不是全局弹窗。
- 卡片展示模型生成的互斥候选项，并固定提供“自定义回答”输入框和发送按钮。
- 若提问卡片已经出现但当前 turn 仍处于 busy，用户可以先点击候选项或提交自定义回答；前端会立即追加一条 `messageKind=queued_user` 的用户气泡展示“等待发送”状态，并锁定该卡片。
- 当前会话退出 busy 后，前端自动发送这条已展示的用户气泡，复用现有 `startTurn` 链路继续，不再生成重复气泡。
- 历史中的旧提问卡片继续展示，但只有当前会话最新且尚未被用户消息回答的提问可提交。

### 3.3 `read_skill` 与 `LOAD_SKILL`（`eventType=TOOL`）
...
- **不再**使用 `NOTICE` + `payload.notice=skill-load` 驱动主界面 busy/轨迹；改为状态机状态 `LOAD_SKILL` + `eventType=TOOL` 与通用工具一致的生命周期（`START` / `DELTA`+`COMPLETE` / `ERROR`）。
...
- **HTTP**：
  - `GET /v1/rest/ai-center/agents` → `AgentInfoDto.showHotQuestions`
  - `GET /v1/rest/ai-center/qa-template?agent-id=&limit=` → 从对应 Agent 的 ClassLoader 读取 `qa-template.json`，随机抽取 `limit` 条；未开启或无资源时返回空 `data`。
...
- `ChatView.vue`：仅负责输入、布局、滚动与基础展示，不直接解析底层协议；「热门问题」由 `showHotQuestions` + `GET /qa-template?agent-id=` 驱动，展示在空会话欢迎区。
- 状态历史展示约定：
  - 实时轮次：`useAgentEventDispatcher` 将**完整 9 态迁移链**（`IDLE` 起点 → busy 段 → `COMPLETED`/`FAILED`/`CANCELLED` 终态）写入当前轮 assistant 尾消息的 `MessageDto.turnSteps`，由 `AgentTurnTimeline` 在气泡内以可折叠时间线展示。
  ...
  - 时间线默认收起；折叠头纵向展示最近两步（每步左侧圆点：已完成实心、进行中加载圆环）；展开后显示完整 9 态链且隐藏折叠缩略头；输入框上方不再展示 `状态1 -> 状态2` 文本链。
- UI 动效约定：
...
- Busy 约定：
  - 非终态 `AGENT_ORCHESTRATING/THINKING/STREAMING_TEXT/CALLING_TOOL/LOAD_SKILL` 统一视为 busy。
  - 终态 `COMPLETED/FAILED/CANCELLED` 退出 busy，恢复输入与历史操作交互。
- 提问答案暂存：
  - 普通输入不做全局队列；busy 期间点击输入框发送按钮仍提示等待当前输出结束，或使用停止按钮中断当前 turn。
  - `ask_question` 卡片例外：卡片出现后即使当前 turn 仍在输出，用户也可以先选择候选项或提交自定义回答。
  - 用户提交提问答案后，前端立即追加一条 `messageKind=queued_user` 的用户气泡展示“等待发送”状态，并暂存 `{agentId, contextId, questionMessageIndex, userMessageIndex, answer}`。
  - 当前会话退出 busy 后，前端将这条已展示的用户气泡改回普通用户消息，并调用原有 `startTurn` 继续会话；同时用 `questionMessageIndex` 锁定来源提问卡片，防止重复提交。
- **Provider 深度思考 vs 最终回答**（`MessageDto.reasoningContent` + `content`）：
  ...
  - **实时双通道**：`ChatService` 从流式 chunk 分别提取 `answerDelta` 与 `reasoningDelta`，经 `MESSAGE/DELTA` 写入 `MessageDto.content` / `MessageDto.reasoningContent`；**仅 answer delta** 触发 `THINKING|AGENT_ORCHESTRATING|CALLING_TOOL|LOAD_SKILL → STREAMING_TEXT`；纯 reasoning delta 保持 `THINKING`。
  ...
  - **中断补偿**：WebSocket 关闭或流错误时，`ChatService#flushPartialAssistantOnInterrupt` 将已推送但未经 Advisor `after()` 落库的 `content` / `reasoningContent` 一并补偿写入记忆（纯思考阶段中断亦保留 `reasoningContent`）；详见 [Agent 记忆机制 — 中断与补偿](../agent记忆机制/README.md#中断与补偿)。
  - **复制/反馈**：仅针对 `content`（最终回答）。

## 5. 后端职责边界

- **通用助手调用子智能体展示**：`universal_assistant` 编排服务 模拟 `call_sub_agent` 工具事件；前端 `dispatcher.ts` 将 `arguments.agentId` 映射为「调用子智能体 {名称}」；子 Agent 流式正文经 `SubAgentStreamBridge` 写入父回合 `streamedContent`。详见 [平台通用助手](../通用助手/README.md)。
- `ChatService`：
  ...
  - 将流式 token、完成、异常、中断统一转换为状态机事件；首 token 进入 `STREAMING_TEXT` 时允许当前状态为 `THINKING`、`AGENT_ORCHESTRATING`、`CALLING_TOOL` 或 `LOAD_SKILL`。
  - 中断时通过 `flushPartialAssistantOnInterrupt` 补偿落库已流式输出的 `content` 与 `reasoningContent`（深度思考双通道）。
  - 路由得到 `resolvedAgentId` 后向 `ActiveChatTurnRegistry` 登记 `contextId + agentId` 流式状态（Redis）；终态收尾时 `unregister`。删除历史等写操作由 `ChatContextService` 查询该登记并返回 409，详见 [Agent 记忆机制 — 流式进行中状态](../agent记忆机制/README.md#流式进行中状态redis)。
- `ChatController`：
  - 负责 websocket 生命周期；
  - 仅转发 envelope 事件；
  - 连接建立后直接进入状态机事件通道，不依赖额外协议参数。
- 插件 `AiAgent`：通过 Alibaba Agent 的 `ToolInterceptor`（`AgentUiToolEventInterceptor`）在工具执行链上发射 `TOOL` 事件；单轮的 `ToolEventEmitter` 经 `RunnableConfig.context` 注入，与 `ChatService` 侧状态机一致。
...
- 挂载官方 Skills 的 Agent：在 `ReactAgent` 上注册 `SkillsAgentHook` 的同时，按顺序注册 `AgentUiSkillLoadToolInterceptor` 与 `AgentUiToolEventInterceptor`。外层在 `read_skill` 上驱动 `LOAD_SKILL` + `TOOL` 事件并写审计；内层对 `read_skill` 跳过 `CALLING_TOOL` 的 TOOL 计数，其它工具照常发射。
