...
- `ChatService`：
  ...
  - 路由得到 `resolvedAgentId` 后向 `ActiveChatTurnRegistry` 登记 `contextId + agentId` 流式状态（Redis）；终态收尾时 `unregister`。删除历史等写操作由 `ChatContextService` 查询该登记并返回 409，详见 [Agent 对话记录机制 §10](../agent对话记录/README.md#10-流式进行中状态redis)。
- `ChatController`：
  - 负责 websocket 生命周期；
  - 仅转发 envelope 事件；
