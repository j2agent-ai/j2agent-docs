...
- **通用助手调用子智能体展示**：`universal_assistant` 编排 Hook 模拟 `call_sub_agent` 工具事件；前端 `dispatcher.ts` 将 `arguments.agentId` 映射为「调用子智能体 {名称}」；子 Agent 流式正文经 `SubAgentStreamBridge` 写入父回合 `streamedContent`。
- `ChatService`：
