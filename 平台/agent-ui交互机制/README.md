...
- `IDLE`：未开始。
- `AGENT_DISPATCHING`：通用助手编排 Hook 开放召回 / 决策阶段；仅 `universal_assistant` 且存在可调用子智能体、且有候选时出现。
- `THINKING`：模型思考中。
...
- `IDLE -> AGENT_DISPATCHING -> THINKING`：`universal_assistant` 被动意图召回完成后进入 ReAct。
...
- `THINKING/STREAMING_TEXT/AGENT_DISPATCHING/CALLING_TOOL/LOAD_SKILL -> CANCELLED`：用户中断或连接关闭。
...
stateDiagram-v2
    ...
    IDLE --> AGENT_DISPATCHING: universal 编排 Hook
    AGENT_DISPATCHING --> THINKING: 调度完成
    ...
    AGENT_DISPATCHING --> CANCELLED: 用户中断/连接关闭
...
- Busy 约定：
  - 非终态 `AGENT_DISPATCHING/THINKING/STREAMING_TEXT/CALLING_TOOL/LOAD_SKILL` 统一视为 busy。
...
- **Provider 深度思考 vs 最终回答**（`MessageDto.reasoningContent` + `content`）：
  ...
  - **实时双通道**：`ChatService` 从流式 chunk 分别提取 `answerDelta` 与 `reasoningDelta`，经 `MESSAGE/DELTA` 写入 `MessageDto.content` / `MessageDto.reasoningContent`；**仅 answer delta** 触发 `THINKING|AGENT_DISPATCHING|CALLING_TOOL|LOAD_SKILL → STREAMING_TEXT`；纯 reasoning delta 保持 `THINKING`。
...
- **通用助手调用子智能体展示**：`universal_assistant` 编排 Hook 模拟 `call_sub_agent` 工具事件；前端 `dispatcher.ts` 将 `arguments.agentId` 映射为「调用子智能体 {名称}」；子 Agent 流式正文经 `SubAgentStreamBridge` 写入父回合 `streamedContent`。详见 [平台通用助手](../通用助手/README.md)。
- `ChatService`：
  ...
  - 将流式 token、完成、异常、中断统一转换为状态机事件；首 token 进入 `STREAMING_TEXT` 时允许当前状态为 `THINKING`、`AGENT_DISPATCHING`、`CALLING_TOOL` 或 `LOAD_SKILL`。
