...
- `IDLE`：未开始。
- `AGENT_SCHEDULING`：通用助手编排 Hook 开放召回 / 决策阶段；仅 `universal_assistant` 且存在可调用子智能体、且有候选时出现。
- `THINKING`：模型思考中。
...
- `IDLE -> AGENT_SCHEDULING -> THINKING`：`universal_assistant` 被动意图召回完成后进入 ReAct。
...
- `THINKING/STREAMING_TEXT/AGENT_SCHEDULING/CALLING_TOOL/LOAD_SKILL -> CANCELLED`：用户中断或连接关闭。
...
stateDiagram-v2
    ...
    IDLE --> AGENT_SCHEDULING: universal 编排 Hook
    AGENT_SCHEDULING --> THINKING: 调度完成
    ...
    AGENT_SCHEDULING --> CANCELLED: 用户中断/连接关闭
