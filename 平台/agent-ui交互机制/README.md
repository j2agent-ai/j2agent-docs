...
- **通用助手调用子智能体展示**：`universal_assistant` 调用 `call_sub_agent` 时，前端 `dispatcher.ts` 将 `arguments.agentId` 映射为「调用子智能体 {名称}」；调用期间子 Agent 流式正文经 `SubAgentStreamBridge` 写入父回合 `streamedContent` 并推送 WebSocket（可在 `CALLING_TOOL` 态切入 `STREAMING_TEXT`）。详见 [平台通用助手](../通用助手/README.md)。
- `ChatService`：
