...
| 中文 | 英文（代码 / API） | 说明 |
|------|-------------------|------|
| 编排服务 | `UniversalAssistantOrchestratorService` | `ChatService` 前置：`orchestrate()` 返回 `CONTINUE` / `DISPATCHED` |
...
4. 至少一次子调用完成 → 返回 **`DISPATCHED`**，**不启动**通用助手主 ReAct
...
flowchart TB
  ...
  Loop -->|已 invoke| Dispatched[DISPATCHED 跳过主 ReAct]
...
与开放召回策略一致；`[]` 时跳过调度循环并返回 `CONTINUE`。详见 `UniversalIntentQueryService` 内 system prompt。
...
4. `DISPATCHED` 时终答来自 bridge flush，**不经**通用助手主 ReAct 二次生成。
